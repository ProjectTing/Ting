...
class TestPostDetailVC: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        print("TestPostDetailVC - viewDidLoad 호출")
        print("Post 데이터 확인:", post ?? "nil")
        setupUI()
        setupNavigationBar()
    ...
    private func setupData() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            // 상태 태그 설정
            let statusTags = [post.positionSearch, post.position]
            self.updateStatusTags(statusTags)
            
            // 활동 가능 시간 설정
            self.availableTimeValueLabel.text = post.availableTime
            self.timeStateValueLabel.text = post.availableTime
            
            // 시급성 설정
            self.urgencyValueLabel.text = post.urgencyLevel
            
            // 기술 스택 설정
            self.updateTechStackTags(post.techstack)
            
            // 프로젝트 타입 태그 설정
            let projectTypes = [post.meeting]
            self.updateProjectTypeTags(projectTypes)
            
            // 상세 내용 설정
            self.descriptionTextView.text = post.detail
    ...
    // MARK: - Tag Updates
    private func updateStatusTags(_ tags: [String]) {
        statusTagsStackView.arrangedSubviews.forEach { $0.removeFromSuperview() }
        tags.forEach { tag in
    ...
    private func updateTechStackTags(_ tags: [String]) {
        techStacksStackView.arrangedSubviews.forEach { $0.removeFromSuperview() }
        tags.forEach { tag in
            let tagView = createTagView(text: tag)
    ...
    private func updateProjectTypeTags(_ tags: [String]) {
        projectTypeStackView.arrangedSubviews.forEach { $0.removeFromSuperview() }
        tags.forEach { tag in
            let tagView = createTagView(text: tag)
